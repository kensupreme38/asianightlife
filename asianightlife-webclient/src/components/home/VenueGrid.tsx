...
import { VenueGridSkeleton } from "@/components/ui/loading-states";
...
export const VenueGrid = ({
  ...
  // Lấy venues với pagination (must be before effects that use isLoading/totalPages)
  const { venues, totalCount, isLoading, isRefreshing } = useVenues({
    selectedCountry,
    selectedCity,
    selectedCategory,
    searchQuery,
  ...
  useEffect(() => {
    if (ignoreUrlPage) return;
    const pageFromUrl = searchParams?.get('page');
    if (!pageFromUrl) return;
    // Chờ load xong trước khi sync URL — tránh loop với page validation khi totalPages=0
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchParams?.toString(), currentPage, isLoading, totalPages, ignoreUrlPage]);
  ...
  }, [currentFiltersKey, searchParams?.toString(), pathname, router, ignoreUrlPage]);

  // Đảm bảo currentPage luôn hợp lệ với totalPages (sau khi đã load)
  useEffect(() => {
    if (isLoading) return;
  ...
  // Scroll to CountrySelector when page changes (homepage only)
  ...
  useEffect(() => {
    ...
    if (previousPageRef.current !== currentPage && previousPageRef.current > 0) {
      ...
      const offsetTop = countrySelector.getBoundingClientRect().top + scrollTop;
      const lenis = (window as Window & { lenis?: { scrollTo: (y: number, opts: { immediate: boolean; duration: number }) => void } }).lenis;
      if (lenis) {
  ...
  return (
    <section className="md:py-12 py-6">
      <div className="md:container px-3">
        ...
        {isLoading && venues.length === 0 ? (
          <VenueGridSkeleton count={ITEMS_PER_PAGE} />
        ) : venues.length > 0 ? (
          <>
            <div
              className={cn(
                "grid grid-cols-2 lg:grid-cols-3 gap-6 items-stretch transition-opacity duration-200",
