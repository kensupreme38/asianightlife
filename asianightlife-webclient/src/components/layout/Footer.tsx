    ...
    <footer className="border-t border-border/40 bg-secondary/20">
      <div className="container py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          {/* Cities */}
          <div>
            <h3 className="font-semibold mb-4">{t('footer.exploreCities').toUpperCase()}</h3>
            <ul className="space-y-2 text-sm">
              {CITIES.map((city) => (
                <li key={city.slug}>
                  <Link href={`/${city.slug}`} className="text-muted-foreground hover:text-primary transition-colors flex items-center gap-2">
                    <CountryFlag country={city.countryCode} size={18} />
                    {city.name}
                  </Link>
                </li>
              ))}
