...
const POPULAR_SUGGESTIONS: Suggestion[] = [
  { type: "category", text: "KTV" },
  { type: "category", text: "Nightclub / clubbing" },
  { type: "country", text: "Singapore" },
  { type: "country", text: "Vietnam" },
  { type: "country", text: "Thailand" },
];
...
export const Header = ({ searchQuery, onSearchChange }: HeaderProps) => {
  ...
  const [venueSearchResults, setVenueSearchResults] = useState<Suggestion[]>([]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/venues?search=${encodeURIComponent(debouncedInput)}&limit=8`, {
      signal: controller.signal,
    })
      .then((r) => r.json())
      .then((data) => {
        setVenueSearchResults(
          (data.venues || []).map((venue: { name: string }) => ({
            type: "venue" as const,
            text: venue.name,
          }))
        );
  ...
  const suggestions = useMemo(() => {
    ...
    const results: Suggestion[] = [...venueSearchResults];
    const seen = new Set(results.map((s) => `${s.type}:${s.text}`));
    ...
    SEARCH_CATEGORIES.forEach((cat) => {
      if (cat.toLowerCase().includes(query) && !seen.has(`category:${cat}`)) {
        results.push({
          type: "category",
          text: cat,
          count: venueStats?.category_stats?.[cat],
        });
        seen.add(`category:${cat}`);
      }
    });

    SEARCH_COUNTRIES.forEach((country) => {
      if (country.toLowerCase().includes(query) && !seen.has(`country:${country}`)) {
        results.push({
          type: "country",
          text: country,
          count: venueStats?.country_stats?.[country],
        });
  ...
  }, [debouncedInput, venueSearchResults, venueStats]);
